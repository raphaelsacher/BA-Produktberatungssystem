...
API_KEY = config.get(
    "API", "api_key"
)  # Annahme: Der API-Schlüssel befindet sich in config.ini unter [API]
...
def priority_combined_search(
    ...
    """
    Priorisierte Kombination von Embedding-Suche und ILIKE mit optionaler Brand-Suche.
    """
...
def embedding_search(
    ...
    """
    Vektorsuche mit Embeddings, zweistufige Markenfilterung (optional):
    ...
    # Erste Stufe: Exakte Markensuche, falls brand angegeben wurde
    query = """
    ...
    # Wenn keine Ergebnisse und brand angegeben wurde, zweite Stufe mit Ähnlichkeitssuche für die Marke
...
def ilike_brand_search(
    ...
    """
    ILIKE-Suche mit optionaler Brand-Filterung:
    1. Exakte Übereinstimmung
    2. ILIKE-Suche für die Marke, falls brand angegeben wurde
    """
    query = """
    SELECT prod_id, text_name, brand_name, price_buynow, text_desc,
    uri_aw_track, uri_m_image, avg_rating, custom3
    ...
    # Wenn keine Ergebnisse und brand angegeben wurde, zweite Stufe mit ILIKE für die Marke
    if brand and not results:
        query = """
        SELECT prod_id, text_name, brand_name, price_buynow, text_desc,
        uri_aw_track, uri_m_image, avg_rating, custom3
...
def fulltext_search(
    ...
    """
    Volltextsuche mit optionaler Brand-Filterung:
    1. Exakte Übereinstimmung
    2. Volltextsuche für die Marke, falls brand angegeben wurde
    """
    ...
    # Wenn keine Ergebnisse und brand angegeben wurde, zweite Stufe mit Volltextsuche für die Marke
    if brand and not results:
        query = """
        SELECT prod_id, text_name, brand_name, price_buynow, text_desc,
        uri_aw_track, uri_m_image, avg_rating, custom3,
...
def find_product(
    ...
    """
    Findet ein Produkt basierend auf dem Suchbegriff und (optional) der Marke unter Verwendung einer
    ...
    for method_name, method_function in search_methods:
        ...
        if query_results:
            ...
            if product_id != 0:
                ...
                logger.info(
                    f"LLM hat keine passende Produkt-ID für die Suchanfrage '{term}' gefunden."
                )
        else:
...
def find_products(
    ...
    for product in products:
        ...
        # Falls "brand" nicht angegeben wurde, wird es auf None gesetzt
    ...
    try:
        ...
        for product in products:
            ...
            if "term" not in product:
                logger.error("Jedes Produkt sollte mindestens 'term' enthalten.")
                return []
...
def search_products_api(
    products: List[Dict[str, str]], x_api_key: str = Depends(verify_api_key)
):
    """
    API-Endpunkt zur Produktsuche.
    Erwartet ein JSON-Array von Produkten, jeweils mit mindestens dem Schlüssel 'term'.
    Falls 'brand' nicht vorhanden ist, wird die Suche ohne Markenfilter durchgeführt.
    """
    # Input-Validierung
    if not products:
    ...
    for product in products:
        if "term" not in product:
            raise HTTPException(
                status_code=400, detail="Jedes Produkt muss 'term' enthalten."
            )
        # Falls 'brand' fehlt, wird es auf None gesetzt
